...
int main()
{
	...
	std::vector<std::shared_ptr<Hittable> > world;
	{
		...
		constexpr u32 Dense = 100;
		constexpr f32 Interval = 2 * Range / Dense;
		constexpr f32 Radius = 0.2 * Interval;
		for (f32 x = -Range; x < Range; x += Interval)
		{
			for (f32 z = -Range; z < Range; z += Interval)
			{
				...
				{
					world.push_back(std::make_shared<Sphere>(center, Radius, std::make_shared<Metal>(vec3(RandomGenerator::uniform_real() * RandomGenerator::uniform_real(), RandomGenerator::uniform_real() * RandomGenerator::uniform_real(), RandomGenerator::uniform_real() * RandomGenerator::uniform_real()), RandomGenerator::uniform_real())));
				}
			}
		}
