...
int main()
{
	...
	std::vector<std::shared_ptr<Hittable> > world;
	{
		...
		world.push_back(std::make_shared<Sphere>(vec3(4, 1, 0), 1.0f, std::make_shared<GravitationalField>(1.0f, vec3(4, 1, 0))));
		world.push_back(std::make_shared<Sphere>(vec3(8, 1, 0), 1.0f, std::make_shared<Metal>(Color::Magenta, 1)));
	...
	//}
	//for (s32 i = 300; i < resolutionX; i+=10)
	//{
	//	//u32 w = RandomGenerator::uniform_int_round(resolutionX / 2, 400);
	//	//u32 h = RandomGenerator::uniform_int_round(resolutionY / 2, 400);
	//	u32 w = static_cast<u32>(i);
	//	u32 h = static_cast<u32>(250);
	//	engine.drawTrajectory(w, h);
	//}
